...
class MagicUtils:
    ...
    def get_llm_config(cls):
        return cls._load_dual_data("llm_settings.txt", DEFAULT_LLM)
    
    @classmethod
    def get_rules_config(cls):
        return cls._load_dual_data("replace_rules.txt", {}) 
    
    @classmethod
    def get_resolutions_config(cls):
        return cls._load_dual_data("resolutions.txt", DEFAULT_RESOLUTIONS)
    
    @classmethod
    def get_logic_config(cls):
        return cls._load_dual_data("logic_rules.json", DEFAULT_LOGICS)


# --- 5. API 路由 (前端 JS 调用的接口) ---
