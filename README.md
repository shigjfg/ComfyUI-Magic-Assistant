...
>    * **Magic Nunchaku FLUX.2 Klein Loader**: 重构并迁移至 ComfyUI 0.29.0 官方 `CoreModelPatcher` 及 `load_device` / `offload_device` 接口，不再使用标准 `ModelPatcher` 的 `device` / `device_id` 接口；功能与使用体验保持一致，同时优化模型加载路径检查与错误处理
>    * **Magic Resolution Resize（图像缩放节点）**: 改用 ComfyUI 0.29.0 官方 `comfy.utils.common_upscale`，替代原有的 `torch.nn.functional.interpolate` 实现；功能与使用体验保持一致，并改进图像与 Latent 的缩放处理
>    * **Magic Photopea（Photopea 图像加载节点）**: 部分迁移至 ComfyUI 0.29.0 官方图像/视频解析管线（`InputImpl.VideoFromFile`、`node_helpers`、`comfy.model_management.intermediate_dtype/device`）；新增支持 Mask Editor 生成的 `clipspace-painted-*.png` 伴随文件，提升图像编辑体验
>    * **Magic Nunchaku FLUX.2 Klein Loader**: Refactored and migrated to ComfyUI 0.29.0's official `CoreModelPatcher` and `load_device` / `offload_device` APIs, replacing the standard `ModelPatcher` `device` / `device_id` interfaces; functionality and user experience remain unchanged, with improved model path validation and error handling
>    * **Magic Resolution Resize**: Migrated to ComfyUI 0.29.0's official `comfy.utils.common_upscale`, replacing the previous `torch.nn.functional.interpolate` implementation; functionality and user experience remain unchanged, with improved image and Latent scaling logic
>    * **Magic Photopea**: Partially migrated to ComfyUI 0.29.0's official image/video parsing pipeline (`InputImpl.VideoFromFile`, `node_helpers`, and `comfy.model_management.intermediate_dtype/device`); added support for `clipspace-painted-*.png` companion files generated by Mask Editor for an improved image-editing experience
>    * **⚠️ 这些更新后的节点依赖 ComfyUI 0.29.0 新版接口，无法在较低版本中使用，请升级至 ComfyUI 0.29.0 或更高版本。**
>    * **⚠️ These updated nodes depend on the new APIs in ComfyUI 0.29.0 and cannot be used with earlier versions. Please upgrade to ComfyUI 0.29.0 or later.**
>
> 2. **💡 新增**: Magic Power LoRA Loader - 可自定义权重调节方式
>    * 在“强力 LoRA 加载器设置”的“界面设置”标签页中，可在箭头按钮与水平滑条两种权重调节方式之间切换
>    * 滑条模式支持自定义步长、最小值、最大值及滑条宽度，并可选择是否显示当前数值，以及是否在拖动至 ±0.02 范围内时自动吸附到 0
>    * 切换为滑条模式后，节点会根据滑条宽度与数值显示设置自动调整最小宽度，确保控件完整可见
>    * 箭头按钮模式继续使用原有的紧凑布局，中间数值仍可点击并直接输入
>    * Added a customizable weight control style under the "Interface Settings" tab in Magic Power LoRA Loader settings, allowing users to switch between arrow buttons and a horizontal slider
>    * Slider mode supports configurable step size, minimum value, maximum value, and slider width, with options to show the current value and snap to zero when dragged within ±0.02
>    * When slider mode is enabled, the node automatically adjusts its minimum width based on the slider width and value display settings to keep all controls fully visible
>    * Arrow-button mode retains the original compact layout, and the center value remains directly editable
><img width="503" height="392" alt="Image" src="https://github.com/user-attachments/assets/697ab4a1-7515-4916-a96b-6e1f2b4df6dc" />
><img width="420" height="497" alt="Image" src="https://github.com/user-attachments/assets/d22af35c-aa95-4672-bcdd-62d2f8c04b0a" />
>
> 3. **🔴 重要提示 / Important Notice**
>    * **ComfyUI 0.29.0 及以上用户**: 请完整更新所有节点文件，以确保新版接口适配正确生效
